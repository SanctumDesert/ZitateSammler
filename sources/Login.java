...
public class Login {
	...
	private void Einloggen()
	{
		...
		try {
			...
			try {
				myStmt = connection.getConnection().prepareStatement("SELECT id, vorname, nachname, passwort, mail, nutzername, admin FROM tbluser WHERE nutzername=? AND passwort=? AND deleted = 0");
